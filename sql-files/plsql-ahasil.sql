...
SELECT SUBSTR(H.HID,1,4) AS HID, H.NAME,
        SUBSTR(GET_MATE_P_PID(H.HID),1,4) AS MATE_P_PID, SUBSTR(H.PID,1,4) AS PID, PN.FHNAME,
        SUBSTR(RF_PN.PID,1,4) AS RF_MATE_P_PID, RF_PN.FHNAME AS RF_FA, RF_PN.MHNAME AS RF_MA,
        SUBSTR(RM_PN.PID,1,4) AS RM_MATE_P_PID, RM_PN.FHNAME AS RM_FA, RM_PN.MHNAME AS RM_MA
    FROM HUMAN H
